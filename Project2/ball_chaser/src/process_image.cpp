...
void drive_robot(float lin_x, float ang_z)
{
   ...
   // Call the command_robot service and pass the requested joint angles
    if (!client.call(srv))
    {
        ROS_ERROR("Operation failed to call service command_robot");
    }	
...
void process_image_callback(const sensor_msgs::Image img)
{
   ...
   // int left = drive_robot(0.0, 0.5);
   // int front = drive_robot(0.5, 0.0);
   // int right = drive_robot(0.0,-0.5);
   // int stop = drive_robot(0.0, 0.0);
